...
class FraudDetector:
    def __init__(self):
        ...
        self.transactions_table_path = str(Path(settings.LOGS_DIR).parent / "lake" / "delta" / "transactions")
    ...
    def _init_delta_table(self):
        ...
        if not path.exists():
            empty_df = self.spark.createDataFrame([], self._get_alert_schema())
    ...
    def _save_alert_to_delta(self, alert: FraudAlert):
        from pyspark.sql import Row
        try:
            logger.info(f"DEBUG: Saving alert {alert.alert_id}")
            data = [Row(
            ...
            df = self.spark.createDataFrame(data, schema=self._get_alert_schema())
            df.write.format("delta").mode("append").save(self.transactions_table_path)
            logger.info(f"DEBUG: Saved alert {alert.alert_id}")
        except Exception as e:
            logger.error(f"DEBUG: Failed to save: {e}")
            logger.error(traceback.format_exc())
    
    def read_transaction_history(self, user_id: str, version: int = None, timestamp: str = None):
        reader = self.spark.read.format("delta")
        if version is not None:
            reader = reader.option("versionAsOf", version)
        elif timestamp is not None:
            reader = reader.option("timestampAsOf", timestamp)
        df = reader.load(self.transactions_table_path)
        return df.filter(F.col("user_id") == user_id)
    ...
    def vacuum_old_files(self, retention_hours: int = 24):
        self.spark.conf.set("spark.databricks.delta.retentionDurationCheck.enabled", "false")
        delta_table = DeltaTable.forPath(self.spark, self.transactions_table_path)
        delta_table.vacuum(retentionHours=retention_hours)
        logger.bind(deletion=True).info(f"VACUUM completed with {retention_hours} hours retention")
    ...
    def process_transaction(self, transaction_data: bytes):
        try:
            ...
            if alerts:
                for alert in alerts:
                    self._save_alert_to_delta(alert)
